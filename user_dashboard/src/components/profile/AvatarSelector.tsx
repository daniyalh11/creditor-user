...
const photoAvatarOptions: AvatarOption[] = [
  // Female avatars
...
// SVG Avatar interfaces and data
interface SvgAvatarStyle {
  id: string;
  name: string;
  example: string;
  category: string;
...
// DiceBear avatar styles with more professional options
const dicebearStyles: SvgAvatarStyle[] = [
...
export function AvatarSelector({
  ...
  const [avatarType, setAvatarType] = useState<"photos" | "svg" | "custom">(
    currentAvatarUrl.includes("dicebear") ? "svg" : 
    currentAvatarUrl.startsWith("data:") ? "custom" : "photos"
  );
  const [photoCategory, setPhotoCategory] = useState<string>(AVATAR_CATEGORIES.FEMALE);
  const [svgCategory, setSvgCategory] = useState<string>("modern");
  ...
  const [dicebearConfig, setDicebearConfig] = useState<DicebearConfig>({
    style: "personas",
  ...
  const generateDicebearUrl = (config: DicebearConfig): string => {
  ...
  const handleSelectPhotoAvatar = (avatar: AvatarOption) => {
    setAvatarType("photos");
    setSelectedAvatar(avatar.src);
  };

  const handleSelectDicebearStyle = (style: string) => {
    setDicebearConfig({
      ...dicebearConfig,
      style
    });
  ...
  const handleImageLoading = (id: string) => {
    setLoadingImages(prev => ({
      ...prev,
      [id]: true
    }));
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className={cn(
        ...
        <div className="flex flex-col items-center justify-center mb-4">
          <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-primary/20 mb-2 shadow-md">
            {isLoading ? (
        ...
        <Tabs 
          ...
          onValueChange={(value) => setAvatarType(value as "photos" | "svg" | "custom")}
        >
