 ...
 */
export const validateAvatarImage = (file: File): { valid: boolean; message?: string } => {
  // Check file type
 ...
 */
export const getRandomProfessionalAvatar = (): string => {
  const gender = Math.random() > 0.5 ? 'male' : 'female';
  const avatars = professionalAvatars[gender as 'male' | 'female'];
