...
interface Question {
  ...
  question: string;
  options?: string[];
  type: 'multiple-choice' | 'true-false' | 'fill-blank' | 'text' | 'matching-pairs' | 'dropdown' | 'numeric' | 'essay' | 'file-upload' | 'hotspot' | 'scenario' | 'drag-drop';
  correctAnswer: string | number | { [key: string]: string } | string[];
  ...
  scenarios?: { situation: string; options: string[]; outcome: string }[];
  dragItems?: string[];
}

type AnswerValue = string | number | { [key: string]: string } | File[] | string[];

// Sample questions based on assessment type
const getQuestions = (assessmentType: string): Question[] => {
...
// Sample questions for proctored exam
const getProctoredExamQuestions = (): Question[] => [
  {
    id: 1,
...
const getDurationInMinutes = (assessmentType: string): number => {
...
export function DemoQuizPage() {
  const { assessmentTitle } = useParams<{ assessmentTitle: string }>();
  ...
  useEffect(() => {
    ...
    if (currentQuestion?.type === 'matching-pairs' && currentQuestion.pairs) {
      const existingAnswer = answers[currentQuestion.id] as { [key: string]: string } | undefined;
      if (existingAnswer) {
  ...
  useEffect(() => {
    ...
    if (currentQuestion?.type === 'drag-drop' && currentQuestion.dragItems) {
      const existingAnswer = answers[currentQuestion.id] as string[] | undefined;
      if (existingAnswer) {
  ...
  const handleHotspotClick = (hotspot: { x: number; y: number; label: string }) => {
  ...
  const isCurrentQuestionAnswered = () => {
    ...
    if (currentQuestion.type === 'matching-pairs') {
      const pairs = answer as { [key: string]: string } | undefined;
      return pairs && Object.keys(pairs).length === currentQuestion.pairs?.left.length;
    }
    
    if (currentQuestion.type === 'essay') {
      const essayText = answer as string;
  ...
  const calculateScore = () => {
    ...
    questions.forEach(question => {
      ...
      if (question.type === 'matching-pairs') {
        ...
        const correctPairs = question.correctAnswer as { [key: string]: string };
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="text-gray-600">
                      {getWordCount((answers[currentQuestion.id] as string) || '')} / {currentQuestion.wordRange?.min} words
                    </div>
                  </div>
                  
                  <div className="text-right text-sm text-gray-500">
                    <div>Characters: {((answers[currentQuestion.id] as string) || '').length}</div>
                  ...
                  {answers[currentQuestion.id] && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                      <p className="text-green-800 text-sm">
                        Selected: <strong>{answers[currentQuestion.id] as string}</strong>
                      </p>
                    </div>
                  )}
                </div>
