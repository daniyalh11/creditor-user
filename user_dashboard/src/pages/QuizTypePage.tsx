...
interface MatchingQuestion extends BaseQuestion {
  ...
  leftColumn: string[];
  rightColumn: string[];
  correctMatches: [string, string][];
}

type QuizQuestion = MultipleChoiceQuestion | SingleChoiceQuestion | BooleanQuestion | TextQuestion | EssayQuestion | MatchingQuestion;
...
export function QuizTypePage() {
  ...
  const quizInfo = quizData[quizType as keyof typeof quizData];
  ...
  const renderQuestion = () => {
    ...
    switch (currentQ.type) {
      case 'multiple':
        const mcqQuestion = currentQ as MultipleChoiceQuestion;
        return (
      ...
      case 'single':
        const scqQuestion = currentQ as SingleChoiceQuestion;
        return (
          <RadioGroup 
            value={userAnswer || ""} 
            onValueChange={(value) => handleAnswer(questionId, value)}
          >
            {scqQuestion.options.map((option, index) => (
              <div key={index} className="flex items-center space-x-2">
      ...
      case 'essay':
        const essayQuestion = currentQ as EssayQuestion;
        return (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Expected length: {essayQuestion.expectedLength}
            </p>
          </div>
        );

      case 'matching':
        const matchingQuestion = currentQ as MatchingQuestion;
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                {matchingQuestion.leftColumn.map((item, index) => (
              ...
              <div>
                ...
                {matchingQuestion.rightColumn.map((item, index) => (
                  <div key={index} className="p-3 bg-gray-50 rounded mb-2 border">
                    {item}
                  </div>
                ))}
